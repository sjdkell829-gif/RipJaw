...
func _ready():
	...
	scale.x = 1.0  # CharacterBody2D siempre en 1
...
func apply_remote_state(data: Dictionary):
	if data.get("type") == "input":
		# Solo mover — no aplicar knockback dos veces
		var input_x = float(data.get("x", 0.0))
		velocity.x = input_x * (stats.move_speed if stats else 200.0)
		if data.get("jumping", false) and is_on_floor():
			velocity.y = stats.jump_force if stats else -500.0
		# Actualizar flip según dirección remota
		if input_x != 0:
		...
		# El hit ya se aplica localmente — ignorar el remoto
		pass
